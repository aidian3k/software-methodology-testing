...
public class OrderService {
	...
	public Order createNewOrder(OrderCreationDTO orderCreationDTO) {
		...
		List<Product> products = productService.getProductsByIds(
			orderCreationDTO.getProductsIds()
		);
		...
		products
			.stream()
			.map(product ->
				Product.builder().availability(product.getAvailability() - 1).build()
			)
	...
	public void cancelOrder(Long orderId) {
		...
		productsOrder
			.stream()
			.map(product ->
				product.toBuilder().availability(product.getAvailability() + 1).build()
			)
			.forEach(productService::updateProduct);
		orderClient.setOrders(
			orderClient
				.getOrders()
				.stream()
	...
	public Order getOrderById(Long orderId) {
		return orderRepository
			.findById(orderId)
			.orElseThrow(() -> new OrderNotFoundException("Order could not be found!")
			);
	}
